    ...
    `;
      ...
      `);
            if (activeProducts.length > 0) {
                productosDbText = activeProducts.map(p => `- ${p.nombre} (${p.categoria}): Presentación ${p.presentacion || 'Estándar'}, Peso ${p.peso ? p.peso + 'kg' : 'N/A'}, Precio: ${Number(p.precio) > 0 ? '$' + Number(p.precio).toFixed(2) : 'A consultar'}, Sede: ${p.sede || 'Ambas'}`).join('\n    ');
            }
        }
        catch (dbErr) {
