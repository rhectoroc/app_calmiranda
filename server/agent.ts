...
const BOSS_NUMBERS = [
  ...
  '584140108030@s.whatsapp.net', // Héctor (Desarrollador / Admin)
...
// ----------------------------------------------------
async function executeTool(name: string, args: any, sessionId: string): Promise<string> {
  try {
    switch (name) {
      case 'SQL_specialist': {
...
// ----------------------------------------------------
export async function handleWebhookMessage(payload: any): Promise<void> {
